...
#[test]
fn control_is_polled_when_the_floor_suppresses_every_batch() {
    let (dim, n) = (64, 2000);
    let idx = plain_index(n, dim, 4);
    let query = unit_vectors(1, dim, 0x51A7_CAFE);
    let mut polls = 0usize;
    let summary = idx
        .try_search_streaming_chunked_controlled(
            &query,
            SearchOptions::new().with_initial_threshold(f32::INFINITY),
            CHUNK,
            |_| panic!("an infinite floor must suppress every batch"),
            || {
                polls += 1;
                if polls == 2 {
                    StreamControl::Stop
