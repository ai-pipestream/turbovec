...
//!    candidate's block was scored, each exactly once, with scores
//!    bitwise identical to a top-k search of the same query batch.
//! 2. `RaiseFloor` takes effect at the next block and only for the
...
fn blocked_index(n: usize, dim: usize, bits: usize, block: usize) -> TurboQuantIndex {
    let mut idx = TurboQuantIndex::with_block_size(dim, bits, block).unwrap();
...
fn completed_stream_is_exactly_the_above_floor_set() {
    ...
    for &bits in &[2usize, 4] {
        let idx = blocked_index(n, dim, bits, 256);
        assert!(idx.sealed_blocks() >= 2);
...
fn no_floor_streams_every_live_slot_once() {
    let (dim, n) = (64, 1500);
    let idx = blocked_index(n, dim, 4, 256);
    let query = unit_vectors(1, dim, 0x0DD5_EED5);

    let mut seen = vec![false; n];
    let summary = idx.search_streaming(&query, SearchOptions::new(), |b| {
        for &slot in b.slots {
...
#[test]
fn raised_floor_applies_from_the_next_block_for_that_query_only() {
    ...
    let summary = idx.search_streaming(&queries, SearchOptions::new(), |b| {
        if b.query_index == 1 {
    ...
    // minus whatever lives in its first-emitted block.
    let first_base = q0_first_base.expect("query 0 emitted at least one batch");
    let block = idx.block_size().unwrap();
    let mut expected: Vec<(u32, i64)> = baseline_above(&idx, &queries, 0, raised)
        .into_iter()
        .filter(|&(_, i)| (i as usize) / block != first_base / block)
        .collect();
    expected.sort_unstable();
    q0_after_raise.sort_unstable();
    assert_eq!(
        q0_after_raise, expected,
        "post-raise stream is not the above-floor set of the remaining blocks",
    );
}
...
fn stop_abandons_the_scan() {
    ...
    assert!(summary.emitted < n, "Stop after one block cannot cover the index");
...
#[test]
fn single_block_index_streams_identically() {
    // No block size configured: the whole index is one block and the
    // stream is one batch per query.
    let (dim, n) = (64, 900);
...
fn nan_raise_panics() {
    let idx = blocked_index(500, 64, 4, 256);
