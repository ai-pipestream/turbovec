...
mod x86_scalar_fallback_tests {
    ...
    /// FORCE_SCALAR_FALLBACK is process-global and the harness runs
    /// tests on parallel threads, so two tests toggling it can switch
    /// each other's kernel mid-search (the #373 hazard). Set-membership
    /// comparison tolerated that; the bit-exact seeded comparison below
    /// cannot, because the kernels differ by a few ULPs and a floor can
    /// sit between them. Every test that stores the flag holds this
    /// lock for its whole store..restore window.
    ...
    fn scalar_fallback_matches_simd_topk() {
        let _serial = KERNEL_SWITCH.lock().unwrap_or_else(|e| e.into_inner());
        let dim = 64;
        let n = 600;
        let nq = 12;
        let k = 16;
